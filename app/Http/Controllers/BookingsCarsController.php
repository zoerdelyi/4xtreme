...
class BookingsCarsController extends Controller
{
    ...
    public function get_working_days($from, $days = 0, $mode, $left_times_start, $left_times_end, $hours_plus = 0, $to = '', $just_days = 0) {
        ...
        if($days != 0 || $to != ''){
            ...
            $get_workingDays = OpeningHoursCars::all();
            $workingDays = [];
    ...
    public function check_time_blocks($start, $end, $date, $left_times_start, $left_times_end, $hours_plus, $interval = 30){
        ...
        $bookings = BookingsCars::where('start_time', 'like', $date.'%')->get();

        $ebedszunet = BookingsSettings::find(8);
        ...
        for( $i = $d_open_time; $i < $d_close_time; $i += $interval) {
            ...
            if($i >= $open_time && $i < $close_time){
                ...
                $paid_total = BookingsCars::where('start_time', 'like', $date.'%')->where('start_time', $open_date)->where('end_time', $close_date)->where('deleted', NULL)->whereNotNull('payment_total')->count();
    ...
    public function update_payment(Request $request){
        ...
        $bookings = BookingsCars::where('bookings_cars.id', $booking_id)
            ->where('deleted', NULL)
            ->first();
    ...
    public function delete_payment(Request $request){
        ...
        $bookings = BookingsCars::where('bookings_tires.id', $booking_id)
            ->where('deleted', NULL)
            ->first();
    ...
    public function update_calendar(Request $request){
        ...
        // KIKAPCSOLVA INNEN
            // $left_times = $booking_settings->find(2);
        ...
        $hours_plus = $booking_settings->find(4);
    ...
    public function get_session(Request $request){
        ...
        $booked = BookingsCars::where('start_time', $open_date)
    ...
    public function insert($booking, $mode = null) {
        ...
        if ($errors) {
            ...
            $bookingCar = new BookingsCars();
            $bookingCar->car_brand_id = $booking[ 'car_brand_id'];
            $bookingCar->car_type_id = $booking[ 'car_type_id'];

            if(!empty($booking['car_brand_other'])){
                $bookingCar->car_brand_other = $booking[ 'car_brand_other'];
            }
            if(!empty($booking['car_type_other'])){
                $bookingCar->car_type_other = $booking[ 'car_type_other'];
            }

            $bookingCar->visitor_id = $booking[ 'visitor_id'];
            $bookingCar->comment = $booking[ 'comment'];
            $bookingCar->start_time = $booking[ 'start_time'];
            $bookingCar->end_time = $booking[ 'end_time'];
            if(isset($booking[ 'dateMins'])){
                $bookingCar->plus_mins = $booking[ 'dateMins'];
            }
            $bookingCar->licence_plate = $booking[ 'licence_plate'];
            $bookingCar->motortip= $booking[ 'motortip'];
            $bookingCar->alvaz= $booking[ 'alvaz'];
            $bookingCar->cm3= $booking[ 'cm3'];
            $bookingCar->teljesitmeny= $booking[ 'teljesitmeny'];
            $bookingCar->confirm_hash = str_random(64);
            // ha admin foglalás, akkor alapértelmezetten legyen confired!
            if($mode == 'admin'){
                $bookingCar->confirmed = 1;
            }
            // gumi tárolás autóknál nem kell!
            // $bookingCar->tire_parking = $booking[ 'tire_parking'];
    ...
    public function edit_get_datas(Request $request){
        ...
        $total_bookings = BookingsCars::where([
        ...
        $cars = BookingsCars::where('bookings_cars.id', $booking_id)
        ...
        ->select(
            ...
            'services_cars.name AS services_name',
        ...
        $cars[0]->comment = nl2br($cars[0]->comment);

        $start_time = $cars[0]['start_time'];
        $start_time = date('Y. m. d. H:i', strtotime($start_time));
        $end_time = $cars[0]['end_time'];
        $end_time = date('H:i', strtotime($end_time));

        $cars[1] = $start_time.' - '.$end_time;

        // összesen hány foglalás van ennél az időpontnál, visszatér az id-kel
        $cars[2] = $total_bookings;
    ...
    public function edit_update(Request $request){
        ...
        $motortip = $request->motortip;
        $alvaz = $request->alvaz;
        $cm3 = $request->cm3;
        $teljesitmeny = $request->teljesitmeny;
        ...
        ->select(
            'bookings_cars.id AS booking_id',
            'bookings_services_cars.id AS bookings_services_cars_id',
        ...
        ->get();
            ...
            if( $cars[0]['bookings_visitors_id'] != 1 ){
        
                // visitors tábla update
                $visitors = Visitors::where('visitors.id', $cars[0]['bookings_visitors_id'])->first();
                if($visitorName){
            ...
            $bookings->motortip = $motortip;
            $bookings->alvaz = $alvaz;
            $bookings->cm3 = $cm3;
            $bookings->teljesitmeny = $teljesitmeny;
            // if($bookingComment){
            ...
            $services = BookingsServicesCars::where('bookings_services_cars.id', $cars[0]['bookings_services_cars_id'])->first();
            if($bookingService){
    ...
    public function edit_delete(Request $request){
        ...
        if($cars){ 
            $cars->deleted = 1;
            $cars->deleted_reason = $reason;
            $cars->updated_at = Carbon::now()->format('Y-m-d H:i:s');
            $cars->save();
